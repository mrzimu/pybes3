...
def test_tof_parse_gid(test_data_dir):
    tof_gid_dict: dict = urt.open(test_data_dir / "ref-gid.root")["tof"].arrays(
        library="np"
    )
    ...
    for tmp_gid in range(tof.N_STRIPS):
        tmp_res = tof.parse_tof_gid(tmp_gid)
    ...
    np_res = tof.parse_tof_gid(np_gid)
    ...
    ak_res = tof.parse_tof_gid(ak_gid)
