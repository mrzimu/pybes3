...
from pybes3.detectors import digit_id
...
def test_mdc_digi_id():
    ...
    ak_wire, ak_layer, ak_is_stereo = (
        digit_id.mdc_id_to_wire(mdc_id_ak),
        digit_id.mdc_id_to_layer(mdc_id_ak),
        digit_id.mdc_id_to_is_stereo(mdc_id_ak),
    )
    assert ak.all(det.get_mdc_digi_id(ak_wire, ak_layer, ak_is_stereo) == mdc_id_ak)
    assert ak.all(digit_id.check_mdc_id(mdc_id_ak))

    # Test numpy
    mdc_id_np = ak.flatten(mdc_id_ak).to_numpy()
    np_wire, np_layer, np_is_stereo = (
        digit_id.mdc_id_to_wire(mdc_id_np),
        digit_id.mdc_id_to_layer(mdc_id_np),
        digit_id.mdc_id_to_is_stereo(mdc_id_np),
    )
    assert np.all(det.get_mdc_digi_id(np_wire, np_layer, np_is_stereo) == mdc_id_np)
    assert np.all(digit_id.check_mdc_id(mdc_id_np))

    # Test uint32
    tmp_id = mdc_id_np[0]
    tmp_wire, tmp_layer, tmp_is_stereo = (
        digit_id.mdc_id_to_wire(tmp_id),
        digit_id.mdc_id_to_layer(tmp_id),
        digit_id.mdc_id_to_is_stereo(tmp_id),
    )
    ...
    assert digit_id.check_mdc_id(tmp_id)
...
def test_emc_digi_id():
    ...
    ak_module, ak_theta, ak_phi = (
        digit_id.emc_id_to_module(emc_id_ak),
        digit_id.emc_id_to_theta(emc_id_ak),
        digit_id.emc_id_to_phi(emc_id_ak),
    )
    assert ak.all(det.get_emc_digi_id(ak_module, ak_theta, ak_phi) == emc_id_ak)
    assert ak.all(digit_id.check_emc_id(emc_id_ak))

    # Test numpy
    emc_id_np = ak.flatten(emc_id_ak).to_numpy()
    np_module, np_theta, np_phi = (
        digit_id.emc_id_to_module(emc_id_np),
        digit_id.emc_id_to_theta(emc_id_np),
        digit_id.emc_id_to_phi(emc_id_np),
    )
    assert np.all(det.get_emc_digi_id(np_module, np_theta, np_phi) == emc_id_np)
    assert np.all(digit_id.check_emc_id(emc_id_np))

    # Test uint32
    tmp_id = emc_id_np[0]
    tmp_module, tmp_theta, tmp_phi = (
        digit_id.emc_id_to_module(tmp_id),
        digit_id.emc_id_to_theta(tmp_id),
        digit_id.emc_id_to_phi(tmp_id),
    )
    ...
    assert digit_id.check_emc_id(tmp_id)
...
def test_tof_digi_id():
    ...
    ak_part, ak_layer_or_module, ak_phi_or_strip, ak_end = (
        digit_id.tof_id_to_part(tof_id_ak),
        digit_id.tof_id_to_layer_or_module(tof_id_ak),
        digit_id.tof_id_to_phi_or_strip(tof_id_ak),
        digit_id.tof_id_to_end(tof_id_ak),
    )
    assert ak.all(
        det.get_tof_digi_id(ak_part, ak_layer_or_module, ak_phi_or_strip, ak_end) == tof_id_ak
    )
    assert ak.all(digit_id.check_tof_id(tof_id_ak))

    # Test numpy
    tof_id_np = ak.flatten(tof_id_ak).to_numpy()
    np_part, np_layer_or_module, np_phi_or_strip, np_end = (
        digit_id.tof_id_to_part(tof_id_np),
        digit_id.tof_id_to_layer_or_module(tof_id_np),
        digit_id.tof_id_to_phi_or_strip(tof_id_np),
        digit_id.tof_id_to_end(tof_id_np),
    )
    assert np.all(
        det.get_tof_digi_id(np_part, np_layer_or_module, np_phi_or_strip, np_end) == tof_id_np
    )
    assert np.all(digit_id.check_tof_id(tof_id_np))

    # Test uint32
    tmp_id = tof_id_np[0]
    tmp_part, tmp_layer_or_module, tmp_phi_or_strip, tmp_end = (
        digit_id.tof_id_to_part(tmp_id),
        digit_id.tof_id_to_layer_or_module(tmp_id),
        digit_id.tof_id_to_phi_or_strip(tmp_id),
        digit_id.tof_id_to_end(tmp_id),
    )
    ...
    assert digit_id.check_tof_id(tmp_id)
...
def test_muc_digi_id():
    ...
    ak_part, ak_segment, ak_layer, ak_channel, ak_gap, ak_strip = (
        digit_id.muc_id_to_part(muc_id_ak),
        digit_id.muc_id_to_segment(muc_id_ak),
        digit_id.muc_id_to_layer(muc_id_ak),
        digit_id.muc_id_to_channel(muc_id_ak),
        digit_id.muc_id_to_gap(muc_id_ak),
        digit_id.muc_id_to_strip(muc_id_ak),
    )
    assert ak.all(det.get_muc_digi_id(ak_part, ak_segment, ak_layer, ak_channel) == muc_id_ak)
    assert ak.all(digit_id.check_muc_id(muc_id_ak))
    ...
    np_part, np_segment, np_layer, np_channel, np_gap, np_strip = (
        digit_id.muc_id_to_part(muc_id_np),
        digit_id.muc_id_to_segment(muc_id_np),
        digit_id.muc_id_to_layer(muc_id_np),
        digit_id.muc_id_to_channel(muc_id_np),
        digit_id.muc_id_to_gap(muc_id_np),
        digit_id.muc_id_to_strip(muc_id_np),
    )
    assert np.all(det.get_muc_digi_id(np_part, np_segment, np_layer, np_channel) == muc_id_np)
    assert np.all(digit_id.check_muc_id(muc_id_np))
    ...
    tmp_part, tmp_segment, tmp_layer, tmp_channel, tmp_gap, tmp_strip = (
        digit_id.muc_id_to_part(tmp_id),
        digit_id.muc_id_to_segment(tmp_id),
        digit_id.muc_id_to_layer(tmp_id),
        digit_id.muc_id_to_channel(tmp_id),
        digit_id.muc_id_to_gap(tmp_id),
        digit_id.muc_id_to_strip(tmp_id),
    )
    assert tmp_part == np_part[0]
    ...
    assert digit_id.check_muc_id(tmp_id)
...
def test_cgem_digi_id():
    ...
    ak_layer, ak_sheet, ak_strip, ak_is_x_strip = (
        digit_id.cgem_id_to_layer(cgem_id_ak),
        digit_id.cgem_id_to_sheet(cgem_id_ak),
        digit_id.cgem_id_to_strip(cgem_id_ak),
        digit_id.cgem_id_to_is_x_strip(cgem_id_ak),
    )
    assert ak.all(
        det.get_cgem_digi_id(ak_layer, ak_sheet, ak_strip, ak_is_x_strip) == cgem_id_ak
    )
    assert ak.all(digit_id.check_cgem_id(cgem_id_ak))

    # Test numpy
    cgem_id_np = ak.flatten(cgem_id_ak).to_numpy()
    np_layer, np_sheet, np_strip, np_is_x_strip = (
        digit_id.cgem_id_to_layer(cgem_id_np),
        digit_id.cgem_id_to_sheet(cgem_id_np),
        digit_id.cgem_id_to_strip(cgem_id_np),
        digit_id.cgem_id_to_is_x_strip(cgem_id_np),
    )
    assert np.all(
        det.get_cgem_digi_id(np_layer, np_sheet, np_strip, np_is_x_strip) == cgem_id_np
    )
    assert np.all(digit_id.check_cgem_id(cgem_id_np))

    # Test uint32
    tmp_id = cgem_id_np[0]
    tmp_layer, tmp_sheet, tmp_strip, tmp_is_x_strip = (
        digit_id.cgem_id_to_layer(tmp_id),
        digit_id.cgem_id_to_sheet(tmp_id),
        digit_id.cgem_id_to_strip(tmp_id),
        digit_id.cgem_id_to_is_x_strip(tmp_id),
    )
    assert tmp_layer == np_layer[0]
    ...
    assert digit_id.check_cgem_id(tmp_id)
