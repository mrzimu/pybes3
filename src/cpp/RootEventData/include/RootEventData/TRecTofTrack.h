#ifndef RootEventData_TRecTofTrack_H
#define RootEventData_TRecTofTrack_H 1
...
class TRecTofTrack : public TObject {
  ...
  public:
    ...
    void setTRecTofTrack( const TRecTofTrack* toftrk ) {
        m_tofTrackID = toftrk->tofTrackID();
        m_trackID    = toftrk->trackID();
        m_tofID      = toftrk->tofID();
        m_status     = toftrk->status();
        m_path       = toftrk->path();
        m_zrhit      = toftrk->zrhit();
        m_ph         = toftrk->ph();
        m_tof        = toftrk->tof();
        m_beta       = toftrk->beta();
        for ( int i = 0; i < 5; i++ ) { m_texp[i] = toftrk->texp( i ); }
        for ( int i = 0; i < 6; i++ ) { m_toffset[i] = toftrk->toffset( i ); }
        for ( int i = 0; i < 6; i++ ) { m_sigma[i] = toftrk->sigma( i ); }
        m_quality   = toftrk->quality();
        m_t0        = toftrk->t0();
        m_errt0     = toftrk->errt0();
        m_errz      = toftrk->errz();
        m_phi       = toftrk->phi();
        m_errphi    = toftrk->errphi();
        m_energy    = toftrk->energy();
        m_errenergy = toftrk->errenergy();
    }
